...
public abstract class XLP extends EnodeBComponent {
	...
	public String getMnc()  {
		String oid = MibReader.getInstance().resolveByName("asLteStkPlmnCfgMnc");
		String output = "";
		try {
			output = snmp.get(oid + ".1").trim();
		} catch (Exception e) {
			output = String.valueOf(GeneralUtils.ERROR_VALUE);
		}
		if (output.equals(String.valueOf(GeneralUtils.ERROR_VALUE))) {
			report.report("Failed getting Mnc", Reporter.WARNING);
			return output;
		}
		String[] hexArray = output.split(":");
		String mnc = "";

		for (int i = 0; i < hexArray.length; i++) {
			if (Integer.parseInt("0" + hexArray[i], 16) != 0)
				mnc += (char) Integer.parseInt(hexArray[i], 16);
		}
		return mnc;
	}

	public String getMcc()  {
		String oid = MibReader.getInstance().resolveByName("asLteStkPlmnCfgMcc");
		String output = "";
		try {
			output = snmp.get(oid + ".1").trim();
		} catch (Exception e) {
		...
		if (output.equals(String.valueOf(GeneralUtils.ERROR_VALUE))) {
			report.report("Failed getting Mcc", Reporter.WARNING);
			return output;
		}
		String[] hexArray = output.split(":");
		String mcc = "";
		...
		for (int i = 0; i < hexArray.length; i++) {
			if (Integer.parseInt("0" + hexArray[i], 16) != 0)
				mcc += (char) Integer.parseInt(hexArray[i], 16);
		}
	...
	public boolean waitForAllRunningAndInService(long timeout) {
		GeneralUtils.printToConsole("will wait for all running " + timeout + " milis");
	 ...
	 * @param acBarringFactorSig
	 * @param acBarringFactorDat
	 * @param acBarringTimeSig
	 * @param acBarringTimeDat
	 * @param acBarringSigAdmin
	 * @param acBarringDatAdmin
	 * @param acBarringEnable
