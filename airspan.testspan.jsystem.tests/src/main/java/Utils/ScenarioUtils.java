...
public class ScenarioUtils {
    ...
    public void fillSetupTable() {
        ...
        if (netspanReachable) {
            systemDefaultsProfiles = StringTools.removeLastXCharsFromString(systemDefaultsProfiles, 2);
     ...
     * @param enbInTest
     */
    public void updateTestVersion(List<EnodeB> enbInTest) {
        for (EnodeB enb : enbInTest)
            enb.updateTestedVer();
    }

    /**
     * Get the EnodeBs ips and set them to the prop file
     */
    private void setEnodeBIpsPropFile(ArrayList<EnodeB> enbInSetup) {
        String enbIps = StringUtils.EMPTY;
        for (EnodeB enb : enbInSetup) {
            enbIps += enb.getNetspanName() + "_IP:" + enb.getIpAddress() + ";";
        }
        enbIps = StringTools.removeLastXCharsFromString(enbIps, 1);
        report.setContainerProperties(0, "enodeB_IP", enbIps);
    }

    /**
     * Get Costumer from SUT and set it to properties
     */
