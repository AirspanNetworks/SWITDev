...
public class P0 extends TestspanTest {
	...
	@Test
	@TestProperties(name = "FTP_software_download_from_new_version_enc", returnParam = {
	...
	public void FTP_software_download_from_new_version_enc() {
		...
		server.setUpgradeServerObject(enbUpgrade.getUpgradeServerIp() + "_" + ServerProtocolType.FTP.toString(),
				enbUpgrade.getUpgradeServerIp(), ServerProtocolType.FTP, enbUpgrade.getUpgradeUser(),
				enbUpgrade.getUpgradePassword());
		setFromNewVersion(server, "enc");
	}

	@Test
	@TestProperties(name = "TFTP_software_download_from_new_version_enc", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	public void TFTP_software_download_from_new_version_enc() {
		EnodeBUpgradeServer server = new EnodeBUpgradeServer();
		server.setUpgradeServerObject(enbUpgrade.getUpgradeServerIp() + "_" + ServerProtocolType.TFTP.toString(),
				enbUpgrade.getUpgradeServerIp(), ServerProtocolType.TFTP, enbUpgrade.getUpgradeUser(),
				enbUpgrade.getUpgradePassword());
		setFromNewVersion(server, "enc");
	...
	@Test
	@TestProperties(name = "SFTP_software_download_from_new_version_enc", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	public void SFTP_software_download_from_new_version_enc() {
		EnodeBUpgradeServer server = new EnodeBUpgradeServer();
		server.setUpgradeServerObject(enbUpgrade.getUpgradeServerIp() + "_" + ServerProtocolType.SFTP.toString(),
				enbUpgrade.getUpgradeServerIp(), ServerProtocolType.SFTP, enbUpgrade.getUpgradeUser(),
				enbUpgrade.getUpgradePassword());
		setFromNewVersion(server, "enc");
	...
	// @Test
	@TestProperties(name = "SFTP_software_download_from_new_version_swu", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	@Test
	@TestProperties(name = "SFTP_software_download_from_new_version_tar", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	@Test
	@TestProperties(name = "SFTP_software_download_to_new_version_enc", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	private void setFromNewVersion(EnodeBUpgradeServer server, String fileType) {
		...
		ispassed = softwareUtiles.createUpgradeImage(server, buildName, imageName, targetVer, hardwareCategory,
				imageType);
		if (!ispassed) {
	...
	public boolean copyFiles(ServerProtocolType protocolType) {
		...
		buildName = (dut.getArchitecture() == EnodeB.Architecture.FSM) ? softwareUtiles.getFSMBuild()
				: softwareUtiles.getXLPBuild();

		boolean copingFile = tempWorker.startCopy();
		if (!copingFile) {
			report.report(
					"wasn't able to copy the file " + buildName + " to " + softwareUtiles.getDestServer().getPath(),
					Reporter.WARNING);
		} else {
	...
	private boolean setPathAndFiles(String fileType, String destinationServerType) {
		...
		softwareUtiles.setDestServer(new File(
				File.separator + File.separator + enbUpgrade.getUpgradeServerIp() + File.separator + FolderType));
		if (!setSourcePath()) {
	...
	public boolean isStatusOK(EnodeB node, String targetVer) {
		...
		if (isUpgrade) {
			...
			if (result.StandbyVersion.equals(targetVer)) {
				...
				report.report("Standby version is " + result.StandbyVersion + " instead of " + targetVer,
						Reporter.FAIL);
				isPass = false;
			}
		}
