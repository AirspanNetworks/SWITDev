...
public class P0 extends TestspanTest {
	...
	/**
	 * @param son
	 *            - true: son enabled, false: son disabled
	 * @param anr
	 *            - 0: Disabled, 1: HO, 2: Periodic
	 * @param dicic
	 *            - true: enabled, false: disabled
	 * @param tbg
	 *            - true: TBG enabled, false: TBG disabled
	 * @param nbr
	 *            - number of inter neighbors (if inter are configured, 1 intra
	 *            is configured as well)
	 * @param threshold
	 *            - null: doesn't change attenuation, Over_a1: change att until
	 *            UE's RSRP0 is over A1 threshold, Below_A2: change att until
	 *            UE's RSRP0 is below A2 threshold,
	 * @param waitForSteady
	 *            - true: wait for steady state after ue connects, false: don't
	 *            wait for steady state
	 * @param enhance
	 *            - true: enable enhance gap in TBG, false: disable enhance gap
	 *            in TBG
	 * @param otdoa
	 *            - active with mib and start measuring with simulate cli
	 *            command
	 * @throws Exception
	 */
	// anr - 0 Disabled, 1 HO, 2 Periodic
	private void genericTest(boolean son, int anr, boolean dicic, boolean tbg, int nbr, Threshold threshold,
			boolean waitForSteady, boolean enhance, boolean otdoa) {
		...
		if (otdoa) {
			...
			if (rnti.equals("defaultValue")) {
				report.report(
						"Did not get rnti value from SNMP - check debugFlags [*] EnablePeriodUeShowLinkTable value",
						Reporter.FAIL);
				return;
			}
		...
		printTablesToReport(expectedTables.getObjectTable(), expectedTables.getReportTable(),
				expectedTables.getCfgTable());
		GeneralUtils.stopLevel();
	...
	// TEST 1
	@Test
	@TestProperties(name = "SonDisable_TBGDisable_AnrDisable_NoNbr", returnParam = {
	...
	// TEST 2
	@Test
	@TestProperties(name = "SonDisable_TBGDisable_AnrDisable_2Inter1Intra", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 10
	@Test
	@TestProperties(name = "SonEnable_TBGEnable_AnrHO_NoNbr", returnParam = { "IsTestWasSuccessful" }, paramsExclude = {
			"IsTestWasSuccessful" })
	...
	private boolean genericTestMultipleUEs(boolean son, boolean dicic, boolean allUes, boolean cdrx) {
		...
		if (!nbr.waitForAllRunningAndInService(60 * 1000)) {
			report.report("EnodeB " + nbr.getNetspanName() + " failed to reach all running and in service",
					Reporter.FAIL);
	...
	public void test20_sonEnableTBGEnableAnrPeriodicLightNoNbrNoChosenUE() {
		...
		if (counter == dynamicArrayUEs.size() - 1) {
			...
			report.report("Number of UEs that received the expected tables is not " + (dynamicArrayUEs.size() - 1)
					+ " - not as expected", Reporter.FAIL);
			reason = "Number of UEs that received the expected tables is not " + (dynamicArrayUEs.size() - 1)
					+ " - not as expected";
		}
	}
	...
	public void test28_sonDisableTBGEnableAnrPeriodicNoNbrUEINLight() {
		...
		report.reportHtml("ue show link before getting measurement from UE in test", dutInTest.lteCli("ue show link"),
				true);
		report.report("Getting measurement tables from UE");
		getTablesFromUEToLocalTable(dm);
		dm.close();
	...
	public void test29_sonDisableTBGDisableAnrPeriodicNoNbrDRX() {
		...
		if (!checkInterEarfcn(dut2)) {
			report.report(dut2.getNetspanName() + " Earfcn not " + interEarfcn + " as expected, stop test.",
					Reporter.FAIL);
	...
	// TEST 34
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrDisable_DicicEnable_1Intra2Inter", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 35
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrPeriodic_DicicEnable_NoNbr", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 36
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrHO_DicicEnable_NoNbr", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 37
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrHO_DicicEnable_1Intra1Inter", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 38
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrDisable_DicicEnable_1Intra1Inter_RFbelowA2", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 39
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrDisable_DicicEnable_1Intra1Inter_RFoverA1", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 40
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrPeriodicSteady_DicicEnable_NoNbr", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 41
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrPeriodicSteady_DicicEnable_NoNbr_RFbelowA2", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 42
	@Test
	@TestProperties(name = "SonDisable_TBGEnable_AnrPeriodicSteady_DicicEnable_NoNbr_RFoverA1", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	// TEST 47
	@Test
	@TestProperties(name = "SonDisable_TBGEnableDual_AnrPeriodicSteady_DicicEnable_NoNbr", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	private boolean setTBGEnable(boolean dualMode) {
		...
		while (numOfCellsInNode > 0) {
			...
			if (!enodeBConfig.cloneAndSetMobilityProfileViaNetSpan(dutInTest,
					dutInTest.getDefaultNetspanProfiles().getMobility(), mobilityParams)) {
				return false;
			}
			numOfCellsInNode--;
		}
	...
	private boolean checkForLight() {
		...
		while (!state.contains("ANR STATE = 5") && System.currentTimeMillis() - time < 3 * 60 * 1000
				&& attenuatorsCurrentValue + step <= maxVal) {
	...
	private void cfgToReport(ArrayList<MeasurementCfg> cfg) {
		...
		for (MeasurementCfg obj : cfg) {
			report.report(
					"Id: " + obj.getId() + ", Object Id: " + obj.getObject_id() + ", Report Id: " + obj.getReport_id());
		}
		GeneralUtils.stopLevel();
	}

	private void printTablesToReport(ArrayList<MeasurementCfgObject> object, ArrayList<MeasurementCfgReport> report_obj,
			ArrayList<MeasurementCfg> cfg) {
	...
	public void end() {
		if (attenuatorRespond) {
			...
			if (dutInTest != null) {
				enodeBConfig.setProfile(dutInTest, EnbProfiles.Son_Profile,
						dutInTest.getDefaultNetspanProfiles().getSON());
				revertMobilityProfiles(dutInTest);
				if (flagNetwork) {
					enodeBConfig.setProfile(dutInTest, EnbProfiles.Network_Profile,
							dutInTest.getDefaultNetspanProfiles().getNetwork());
					if (needsToReboot) {
	...
	private void revertMobilityProfiles(EnodeB dutInTest2) {
		...
		while (numOfCells > 0) {
			...
			enodeBConfig.setProfile(dutInTest2, EnbProfiles.Mobility_Profile,
					dutInTest2.getDefaultNetspanProfiles().getMobility());
			numOfCells--;
		}
	}
	...
	// OTDOA TESTS
	@Test
	@TestProperties(name = "OTDOA1_BasicOperation", returnParam = { "IsTestWasSuccessful" }, paramsExclude = {
			"IsTestWasSuccessful" })
