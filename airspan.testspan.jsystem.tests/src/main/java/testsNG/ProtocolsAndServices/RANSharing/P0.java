...
public class P0 extends TestspanTest {
	...
	public void SharedENB_UEattachingFunctionalTest_TwoUEsDifferentPLMN() {
		...
		if (currentMME == null){
			...
			report.report("MME Found: " + currentMME.getS1IpAdress()+ " PLMN1: " + currentMME.getPLMNs().get(0) + " PLMN2: "+ currentMME.getPLMNs().get(1));
		if(otherMME == null){
			...
			report.report("MME Found: " + otherMME.getS1IpAdress()+ " PLMN3: " + otherMME.getPLMNs().get(0));
		GeneralUtils.stopLevel();
	...
	@Test
	@TestProperties(name = "UEEntry_DifferentPLMN_SameMME", returnParam = {
			"IsTestWasSuccessful" }, paramsExclude = { "IsTestWasSuccessful" })
	...
	public void SharedMME_UEattachingFunctionalTest_TwoUEsDifferentPLMN()  {
		...
		boolean isUesconnected = false;
			ArrayList<UE> UEsForDUT = SetupUtils.getInstance().getallUEsPerEnodeb(dut1);
			if (enableSpecificUE(UEsForDUT, currentMME.getPLMNs().get(0))) isUesconnected = true;
			if (enableSpecificUE(UEsForDUT, currentMME.getPLMNs().get(1))) isUesconnected = true;
	...
	private MME getOtherMME(MME currentMME) {
		for (MME mme : EPC.getInstance().MME)
			if (!mme.getS1IpAdress().equals(currentMME.getS1IpAdress()))
				return mme;
		return null;
	}
	...
	private boolean settingEnodeB(MME currentMME, int PLMNindex, EnodeB enb)  {
		...
		GeneralUtils.startLevel("Setting EnodeB " + enb.getNetspanName() + " with " + currentMME.getPLMNs().get(PLMNindex) );
		boolean setPLMN = false;
		try {
			setPLMN = setS1AndPLMN(enb, currentMME, plmns);
		if (!setPLMN)
			report.report("couldn't set the enodeB " + enb.getNetspanName() + " with the plmn " + currentMME.getPLMNs().get(PLMNindex), Reporter.WARNING);
		} catch (Exception e) {
	...
	private boolean settingEnodeB(MME currentMME, EnodeB enb) {
		...
		GeneralUtils.startLevel("Setting EnodeB " + enb.getNetspanName() + " with " + currentMME.getPLMNs().get(0) + " and "+ currentMME.getPLMNs().get(1));
			boolean setPLMN;
					setPLMN = setS1AndPLMN(enb, currentMME, plmns);
			if (!setPLMN)
				report.report("couldn't set the enodeB " + enb.getNetspanName() + " with the plmns: " + currentMME.getPLMNs().get(0)+ " , "+ currentMME.getPLMNs().get(1), Reporter.WARNING);
				res = false;
		} catch (Exception e) {
			...
			if (ue.getPlmnByImsi().equal(plmn)) {
				if (!expectedUEList.contains(ue)){
						if (ue.start()){
							...
							report.report("Couldn't enable UE: " +ue.getName() + " (imsi: " +  ue.getImsi()+ ") ", Reporter.WARNING);
	...
	private boolean setS1AndPLMN(EnodeB dut, MME currentMME, ArrayList<Plmn> plmnList){
		...
		np.setProfileName(dut.getDefaultNetspanProfiles().getNetwork() + "_" + GeneralUtils.getPrefixAutomation());
		np.setMMEIP(currentMME.getS1IpAdress());
	...
	private void SetProfileBack(EnodeB enb){
		if (!enodeBConfig.setProfile(enb, EnbProfiles.Network_Profile, enb.getDefaultNetspanProfiles().getNetwork())) {
			...
			report.report("Network profile " + enb.getDefaultNetspanProfiles().getNetwork() + " was set on enodeB " + enb.getNetspanName());
	}

	private void checkIfRightPlmn(ArrayList<Plmn> enodeBplmns, UE ue)  {
		...
		if (hPlmn != null && selPlmn != null){
			...
			if(homePlmn.equal(uePlmn)){
				report.report("The UE: " + ue.getLanIpAddress() + " [ "+ ue.getImsi()+"] home Plmn is " + homePlmn.show()+" as expected");
			}
			else 
				report.report("The UE: " + ue.getLanIpAddress() + " [ "+ ue.getImsi()+"] home Plmn is " +homePlmn.show()+" and not "+ uePlmn.show(), Reporter.FAIL);
			if (enodeBplmns.size()>1){
				boolean res = false;
				for (Plmn tempPlmn : enodeBplmns) {
					if (tempPlmn.equal(selectedPlmn)) {
						report.report("The UE: " + ue.getLanIpAddress() +" [ "+ ue.getImsi()+"] selected Plmn is " + selectedPlmn.show()+"  as expected");
						res = true;
						break;
					}		
				}
				if (!res)
				report.report("The UE: " + ue.getLanIpAddress() + " [ "+ ue.getImsi()+"] selected Plmn is " + selectedPlmn.show()+ " and not " + enodeBplmns.get(0)+ " OR " + enodeBplmns.get(1) , Reporter.FAIL);	
			}
			else{
				if (enodeBplmns.get(0).equal(selectedPlmn))
					report.report("The UE: " + ue.getLanIpAddress() +" [ "+ ue.getImsi()+"] selected Plmn is " + selectedPlmn.show()+"  as expected");
				else 
					report.report("The UE: " + ue.getLanIpAddress() + " [ "+ ue.getImsi()+"] selected Plmn is " + selectedPlmn.show()+ " and not " + enodeBplmns.get(0) , Reporter.FAIL);
			}
		}
		else 
	...
	private void checkingMIBs(EnodeB enb1, EnodeB enb2) {
		for (String mib : passCriteria) {
			...
			if (result == (1*expectedUEList.size())) {
				report.report(
						"[Pass Criteria: ] EnodeB: " + enb1.getName()+" + " + enb2.getName() + " counter " + mib + " value is: " + result);
			} else {
				report.report("[Pass Criteria: ] EnodeB: " + enb1.getName()+" + " + enb2.getName() + " counter " + mib + " value is: " + result,
						Reporter.WARNING);
			}
		}
	}
